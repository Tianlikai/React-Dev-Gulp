...
function formatterHintWithClick(params) {
    let div = formatterTooltip(params, `单击可下钻. 双击可查看项目详情`);
    return div;
}

// 提示可双击
function formatterHintWithDbClick(params) {
    let div = formatterTooltip(params, '双击可查看项目详情');
    return div;
}
 ...
 * @return {返回单个item配置对象}
 */
function getSeriesItem({ name, type, barMaxWidth, barMinHeight, colorIndex, show, position, color, data }) {
    return {
        name: name,
        type: type,
