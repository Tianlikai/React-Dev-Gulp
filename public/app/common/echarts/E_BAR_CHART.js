...
import { BAR_SeriesItemType, BAR_LabelPosition, BAR_MaxWidth, BAR_MinHeight, DataZoomWidth, ColorArray } from './EchartType'
 ...
 * @return {obj:option}   
 */
function getBarChartOpt({ type, chartData, from, isShow, WhetherTheTrip }) {
    let {
        legend,
        x_name: seriesName,
        y_name,
    ...
    let flag = y.every((ele) => { // 判断是否所有的刻度标签都为空
        return ele === '';
    });
    let option = {
        tooltip: {
    ...
    if (isShow) { // 是否显示数值
        let mainAxisLen = y.length // y轴显示的数目
