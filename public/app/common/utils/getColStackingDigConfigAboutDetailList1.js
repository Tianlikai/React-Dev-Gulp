/**
   ...
   * @param {*} count 
   */
const getColStackingDigConfigAboutDetailList1 = (chartData, seriesData, params, count) => {
    let list = [];
    for (let i = 0, j = seriesData[0].length; i < j; i++) {
        let item = [];
        item.push(chartData.x[i]); // 名称
        if (seriesData[0][i] === '-') {
            item.push('-');
            item.push('-');
...
export default getColStackingDigConfigAboutDetailList1;